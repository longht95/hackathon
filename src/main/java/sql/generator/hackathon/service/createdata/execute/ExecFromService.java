	...
	private void processInit(ParseObject parseObject, ReturnObjectWhere returnObjWhere) {
		...
		tables.stream().forEach(x -> {
			...
			.forEach(y -> {
				...
				String tableAliasColumnName = tableName + Constant.STR_DOT + aliasName + Constant.STR_DOT + arrTableColumn[2];
...
//				}
	...
	private void processCalcMappingKey(Map<String, List<Cond>> columnMap) throws SQLException {
		...
		for (Map.Entry<String, List<Cond>> e : columnMap.entrySet()) {
			...
			List<String> validOfCol = returnObjWhere.getValueMappingTableAliasColumn().get(tableAliasColumnName).getValidValueForColumn();
			if (validOfCol == null || validOfCol.isEmpty()) {
	...
	private Map<String, List<Cond>> calcLastMapping(Map<String, List<Cond>> columnMap) {
		...
		Set<String> visited = new HashSet<>();
		columnMap.entrySet().forEach(x -> {
			String tableAliasColumnName = x.getKey();
			if (!visited.contains(tableAliasColumnName)) {
				List<Cond> mapping = x.getValue();
				List<String> valuesInWhereCondition = returnObjWhere.getValueMappingTableAliasColumn().get(tableAliasColumnName).getValidValueForColumn();
